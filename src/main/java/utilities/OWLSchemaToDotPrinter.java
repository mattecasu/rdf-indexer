...
public class OWLSchemaToDotPrinter {
	...
	public enum DIRECTIONS {TB, BT, LR, RL};
	public enum NAMETYPE {QNAME, LABEL};
	...
	public OWLSchemaToDotPrinter(String fileLocation, NAMETYPE nameType, String langTag, DIRECTIONS dir, String color) {
	...
	public String getDot(String rdfFormat) {
		...
		while(properties.hasNext()) {
			...
			if (range.asClass().isUnionClass()) {
				...
				rangeLabel = range.getLocalName();
			else
				rangeLabel = getNameOrLabel(range);


			if(property.isObjectProperty()) {
				if (domainLabel != null && rangeLabel != null) {
					String toAdd = "\"" + domainLabel + "\"" + "->" + "\"" + rangeLabel + "\"" + " [label=\"" + getNameOrLabel(property) + "\"]\n";
					toPrint += toAdd;
				...
				if (domainLabel != null && rangeLabel != null) {
					String toAdd = "\"" + domainLabel + "\"" + "->" + "\"" + getNameOrLabel(property) + " ("+ rangeLabel +")" + "\" [" + "arrowhead=dot]\n";
					toAdd += "\"" + getNameOrLabel(property) + " ("+ rangeLabel +")" + "\"" + " [shape=plaintext]\n";
					toPrint += toAdd;
					printedClasses.add(domainLabel);
					//printedClasses.add(rangeLabel);
				}
		...
		for(UnionClass cl : unionClasses) {
			for(OntClass operand : cl.listOperands().toList()) {
				...
				Pair<String, String> couple = new ImmutablePair<String, String>(operandLabel, unionClassLabel);
				couples.add(couple);
			}
		}
		for(Pair<String, String> couple : couples) {
			toPrint += String.format("\"%s\" -> \"%s\" [style=dotted]\n", couple.getLeft(), couple.getRight());
	...
	private void recursivePrint(OntClass current) {
		...
		while(superClasses.hasNext()) {
			...
			if(superClassesToIgnore.contains(superClass.getURI()))
				continue;
			if(superClass.isUnionClass()) {
				String toAdd = "\"" + getNameOrLabel(current) + "\" -> \"" + getUnionLabel(superClass) + "\" [style=dotted]\n";
				toPrint += toAdd;
				printedClasses.add(getNameOrLabel(current));
				printedClasses.add(getUnionLabel(superClass));
				Pair<String, String> couple = new ImmutablePair<String, String>(getNameOrLabel(current), getUnionLabel(superClass));
				couples.add(couple);
			}
			else {
				String toAdd = "\"" + getNameOrLabel(current) + "\" -> \"" + getNameOrLabel(superClass) + "\" [style=dotted]\n";
				toPrint += toAdd;
				printedClasses.add(getNameOrLabel(current));
	...
	public static void main(String... args) throws IOException {
		...
		OWLSchemaToDotPrinter printer = new OWLSchemaToDotPrinter(
				"file://" + inFile,
				NAMETYPE.LABEL,
				"en",
				DIRECTIONS.LR,
